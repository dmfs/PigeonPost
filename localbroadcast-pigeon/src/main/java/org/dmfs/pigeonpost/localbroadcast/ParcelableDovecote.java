 ...
 * <p>
 * {@link ParcelableCage}s are good for communication within the same process. Note that sent {@link Pigeon}s are lost if there are no active {@link
 * ParcelableDovecote}s available.
 *
 * @author Marten Gajda
 */
public final class ParcelableDovecote<T extends Parcelable> implements Dovecote<T>
{
     ...
     */
    public ParcelableDovecote(@NonNull Context context, @NonNull String name, @NonNull OnPigeonReturnCallback<T> callback)
    {
        mContext = context;
        mName = name;
    ...
    public Cage<T> cage()
    {
        return new ParcelableCage<T>(new Intent(mName));
    }
